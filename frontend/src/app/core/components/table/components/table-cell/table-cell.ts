...
export class TableCell {
  ...
  get tagValue(): string {
    if (this.columnKey === 'confirmed') return this.value ? 'Confirmed' : 'Not Confirmed';
    if (this.columnKey === 'blocked') return this.value ? 'Blocked' : 'Unblocked';
    return '';
  }

  get tagSeverity(): 'success' | 'danger' {
    if (this.columnKey === 'confirmed') return this.value ? 'success' : 'danger';
